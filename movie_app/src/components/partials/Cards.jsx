...
const Cards = ({data, title}) => {
  return data.length > 0 ?  
      <div className='flex flex-wrap items-center justify-center w-full overflow-hidden gap-12 h-fit bg-[#1f1e24]'>
    {
      data.map((card, i) => <Link to={`/${card.media_type || title }/details/${card.id}`} key={i} className='relative w-[30vh] h-[65vh]'>
